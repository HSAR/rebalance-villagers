...
/**
 * This utility class allows item and block IDs to be retrieved using the names
 * specified for them in Minecraft's source code, and also allows custom names
 * to be associated with IDs.
 * 
...
public class ItemIDGetter
{
     ...
     * 
     * @param name
     *            - the name of the desired item or block
     * @return The int value of the ID of this item or block, or -1 if no such
     *         item or block exists.
     */
    public static Item getItemOrBlock(String name)
    {
        ...
        if (item == null)
        {
            ...
            {
                // Squelch exception because it simply means name wasn't a
                // numeric ID.
            }
        }
    ...
    /**
     * Registers a custom item or block name to the given ID. This can also
     * overwrite a previously registered name.
     * 
     * @param name
     *            - the desired item or block name
     * @param id
     *            - the id this name is meant to represent
     */
    ...
    static
    {
        ...
        {
            // This is so we have an idea of what goes wrong if something does
            // go wrong.
            // The alternative, ExceptionInInitializerError, is not particularly
            // helpful.
            e.printStackTrace();
