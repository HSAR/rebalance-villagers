...
import net.minecraft.server.v1_7_R3.Tuple;
...
public class ConfigLoader
{
    ...
    private static final String CONFIG_REMOVE_OFFERS = CONFIG_OFFER_REMOVAL
            + ".remove-offers";
    private static final String CONFIG_REMOVAL_CHANCE = CONFIG_OFFER_REMOVAL
            + ".removal-chance";
    ...
    /**
     * @param handle
     *            - the RebalanceVillagers plugin to load the config for.
     */
    ...
    void applyConfig()
    {
        BalancedVillager.setOfferRemoval(getConfig().getBoolean(CONFIG_REMOVE_OFFERS,
                true));
        ...
        if (removalMinimum > removalMaximum)
            getLogger().info(
                    "Warning: '" + removalMin + "' is greater than '" + removalMax
                            + "'! " + CONFIG_WARNING_POSTFIX);
        
        String defaultOfferCount = CONFIG_OFFER_GENERATION + ".default-offer-count";
        BalancedVillager.setDefaultOfferCount(validateMinimumOfOne(
                getConfig().getInt(defaultOfferCount, 1), defaultOfferCount,
                CONFIG_WARNING_POSTFIX));
        String newOfferCount = CONFIG_OFFER_GENERATION + ".new-offer-count";
        BalancedVillager.setNewOfferCount(validateMinimumOfOne(
                getConfig().getInt(newOfferCount, 1), newOfferCount,
                CONFIG_WARNING_POSTFIX));
        String generationTicks = CONFIG_OFFER_GENERATION + ".generation-ticks";
        BalancedVillager.setGenerationTicks(validateMinimumOfOne(
                getConfig().getInt(generationTicks, 60), generationTicks,
                CONFIG_WARNING_POSTFIX));
        ...
        BalancedVillager.setParticleTicks(validateMinimumOfOne(
                getConfig().getInt(particleTicks, 200), particleTicks,
                CONFIG_WARNING_POSTFIX));
        ...
        // BalancedVillager.setMaxHealth(validateMinimumOfOne(getConfig().getInt(maxHealth,
        // 20), maxHealth, CONFIG_WARNING_POSTFIX));
        plugin.allowDamage = getConfig().getBoolean(CONFIG_GENERAL + ".allow-damage",
                true);
        ...
        if (plugin.allowedProfessions.length < 1)
        {
            plugin.allowedProfessions = RebalanceVillagers.DEFAULT_ALLOWED_PROFESSIONS;
            getLogger().info(
                    "Warning: '" + professions + "' must contain at least one number! "
                            + CONFIG_WARNING_POSTFIX);
        }
        
        String checkAttempts = CONFIG_SHOPKEEPERS + ".times-to-check";
        ShopkeeperWaiter.shopkeeperCheckAttempts = validateMinimumOfOne(getConfig()
                .getInt(checkAttempts, 5), checkAttempts, CONFIG_WARNING_POSTFIX);
        String checkDelay = CONFIG_SHOPKEEPERS + ".time-between-checks";
        ShopkeeperWaiter.shopkeeperCheckDelay = validateMinimumOfOne(
                getConfig().getInt(checkDelay, 50), checkDelay, CONFIG_WARNING_POSTFIX);
    }
    ...
    void applyOfferConfig()
    {
        ...
        if (!itemOrBlockExists(currencyItemOrBlock, currencyName, CONFIG_CURRENCY_ITEM,
                OFFERS_WARNING_POSTFIX))
            currencyItemOrBlock = Items.EMERALD;
        
        ItemIDGetter.registerName(CONFIG_CURRENCY_ITEM, currencyItemOrBlock); // Allow
                                                                              // users
                                                                              // to
                                                                              // use
                                                                              // "currency-item"
        ...
        // This is needed for the hardcoded default gold offer to work properly,
        // so we prepare it ahead of time.
        HashMap<Item, Tuple> buyValues = new HashMap<Item, Tuple>();
        buyValues
                .put(Items.GOLD_INGOT, new Tuple(Integer.valueOf(2), Integer.valueOf(3)));
        
        // Get offers
        HashMap<Integer, PotentialOffersList> offersByProfession = new HashMap<Integer, PotentialOffersList>();
        ConfigurationSection professions = getOfferConfig().getConfigurationSection(
                CONFIG_POTENTIAL_OFFERS);
        if (!pathContentsExists(professions, CONFIG_POTENTIAL_OFFERS,
                OFFERS_WARNING_POSTFIX))
        {
            // The only possible offer at this point is the default, which is
            // all we will assign.
        ...
        for (String name : professionNames)
        {
            ...
            if (professionId == null)
            {
                ...
                catch (NumberFormatException e)
                {
                    getLogger().info(
                            "Warning: '" + path
                                    + "' is not a recognized villager profession! "
                                    + OFFERS_WARNING_POSTFIX);
                    continue;
                }
            }
            ...
            ConfigurationSection configOthers = getOfferConfig().getConfigurationSection(
                    configOtherPath);
            if (pathExists(configOtherPath, getOfferConfig(), OFFERS_WARNING_POSTFIX)
                    && offersExist(configOthers, configOtherPath))
            {
        ...
        // Get buy values
        ConfigurationSection configBuyValues = getOfferConfig().getConfigurationSection(
                CONFIG_BUY_VALUES);
        ...
        ConfigurationSection configSellValues = getOfferConfig().getConfigurationSection(
                CONFIG_SELL_VALUES);
        if (pathContentsExists(configSellValues, CONFIG_SELL_VALUES,
                OFFERS_WARNING_POSTFIX))
        {
    ...
    /**
     * Validates that the specified value is no less than 1. If value is less
     * than 1, a warning is issued.
     * 
     * @param value
     *            - the value to validate
     * @param path
     *            - the path to the value. Used for the warning message.
     * @param postfix
     *            - this String is appended to the warning message
    ...
    /**
     * Validates that the specified value is a double value between 0.0 and 1.0,
     * inclusive. A warning is issued if these requirements are not met.
     * 
     * @param path
     *            - the path to the value to check
     * @param config
     *            - the ConfigurationSection containing this path
     * @param postfix
     *            - this String is appended to the warning message
     * @return The value at the given path, if all conditions are met. If the
     *         value is not a double, or it is less than 0.0, 0.0 is returned.
     *         If the value is greater than 1.0, 1.0 is returned.
     */
    private double validateProbability(String path, ConfigurationSection config,
            String postfix)
    {
        double probability;
        ...
        if (!config.isDouble(path))
            getLogger().info(
                    "Warning: '" + path + "' must be a decimal, such as 1.0! " + postfix);
        ...
        if (probability < 0 || probability > 1)
        {
            ...
            getLogger().info(
                    "Warning: '" + path + "' must be between 0.0 and 1.0 (inclusive)! "
                            + postfix);
        }
    ...
    /**
     * Returns whether or not the given path exists in the given
     * ConfigurationSection. If it does not, a warning is issued.
     * 
     * @param path
     *            - the path to check
     * @param config
    ...
    /**
     * Returns whether or not the given id is greater than or equal to 0. If it
     * is not, a warning is issued, stating that the specified item/block name
     * is unknown.
     * 
     * @param id
     *            - the id to check
     * @param name
     *            - the name of the item. Used for the warning message.
     * @param path
     *            - the path to the value. Used for the warning message.
     * @param postfix
     *            - this String is appended to the warning message
     ...
     */
    private boolean itemOrBlockExists(Object itemOrBlock, String name, String path,
            String postfix)
    {
        if (itemOrBlock == null)
        {
            getLogger().info(
                    "Warning: '" + name + "' at '" + path + "' is an unknown item! "
                            + postfix + " Use a data value if necessary.");
            return false;
        }
        return true;
    }
    ...
    /**
     * Returns whether or not the given Integer List could be interpreted as a
     * range. Issues a warning if it cannot.
     * 
     * @param range
     *            - the Integer List to check
     * @param path
     *            - the path to the value. Used for the warning message.
     * @param postfix
     *            - this String is appended to the warning message
     * @return True if range exists and has a size of at least 1, false
     *         otherwise.
     */
    private boolean rangeExists(List<Integer> range, String path, String postfix)
    {
        ...
        {
            getLogger().info(
                    "Warning: '" + path + "' does not specify a range! " + postfix);
            return false;
        }
        return true;
    }
    ...
    /**
     * Returns whether or not the given ConfigurationSection (intended to
     * represent offers) exists. If a user specifies "nothing" for an offer
     * list, YAML does not treat the path as a ConfigurationSection. This method
     * previously issued warnings in such a scenario, but it proved annoying.
     * 
     * @param value
     *            - the section to check
     * @param path
     *            - the path to the value. Previously used for the warning
     *            message.
     * @return False if value is null, true otherwise.
     */
    private boolean offersExist(ConfigurationSection value, String path)
    {
        if (value == null)
        {
            ...
            // getLogger().info("Notice: '" + path +
            // "' contains no offers. If this is not intentional, check your offers.yml file.");
            return false;
        }
        return true;
    }
    ...
    /**
     * Allows easy access to a key's name, by trimming the path. I made this
     * because I misunderstood how the getKeys method functions.
     * 
     * @param path
     *            - the path representing this key
     * @param header
    ...
    /**
     * Attempts to populate the given SimpleOffer list with the contents of the
     * path, if offers are defined there.
     * 
     * @param targetList
     *            - the list to populate
     * @param path
     *            - the location of the offers to populate the list with
     */
    private void populateOffersList(ArrayList<SimpleOffer> targetList, String path)
    {
        ConfigurationSection configOffers = getOfferConfig()
                .getConfigurationSection(path);
        if (pathExists(path, getOfferConfig(), OFFERS_WARNING_POSTFIX)
                && offersExist(configOffers, path))
        {
    ...
    /**
     * Populates the given SimpleOffer list with the contents of
     * potentialOffers.
     * 
     * @param targetList
     *            - the list to populate
     * @param potentialOffers
     *            - keys representing offers to be added to the list
     * @param pathHeader
     *            - the parent path of the keys
     */
    private void populateSimpleOffers(ArrayList<SimpleOffer> targetList,
            Set<String> potentialOffers, String pathHeader)
    {
        for (String name : potentialOffers)
        {
            ...
            {
                double probability = validateProbability(path, getOfferConfig(),
                        OFFERS_WARNING_POSTFIX);
    ...
    /**
     * Populates the given CustomOffer list with the contents of
     * potentialOffers.
     * 
     * @param targetList
     *            - the list to populate
     ...
     *            - the parent path of the keys
     */
    private void populateCustomOffers(ArrayList<CustomOffer> targetList,
            Set<String> potentialOffers, String pathHeader)
    {
        for (String name : potentialOffers)
        {
            ...
            if (pathExists(configBuyAPath, getOfferConfig(), OFFERS_WARNING_POSTFIX)
                    && pathExists(configSellPath, getOfferConfig(),
                            OFFERS_WARNING_POSTFIX))
            {
                ItemStackProducer buyA = loadItemStackProducer(configBuyAPath);
                ItemStackProducer buyB = loadItemStackProducer(path + ".buyB"); // Will
                                                                                // be
                                                                                // null
                                                                                // if
                                                                                // nonexistent
                ItemStackProducer sell = loadItemStackProducer(configSellPath);
                double probability = validateProbability(path + ".probability",
                        getOfferConfig(), OFFERS_WARNING_POSTFIX);
                
                if (buyA != null && sell != null)
                    targetList
                            .add(new CustomOffer(buyA, buyB, sell, (float) probability));
            }
        }
    }
     ...
     * 
     * @param path
     *            - the location of the ItemStackProducer
    ...
    private ItemStackProducer loadItemStackProducer(String path)
    {
        ...
        if (getOfferConfig().contains(itemPath))
        {
            ...
            if (itemOrBlockExists(item, itemName, path, OFFERS_WARNING_POSTFIX))
            {
                if (getOfferConfig().contains(itemstackPath))
                    getLogger().info(
                            "Warning: '" + itemstackPath + "' was not checked, as '"
                                    + itemPath + "' was specified. "
                                    + OFFERS_WARNING_POSTFIX);
                
                List<Integer> amount = getOfferConfig().getIntegerList(amountPath);
                int minimum, maximum;
                if (getOfferConfig().contains(amountPath)
                        && rangeExists(amount, amountPath, OFFERS_WARNING_POSTFIX))
                {
                ...
                if (getOfferConfig().contains(enchantPath)
                        && rangeExists(enchant, enchantPath, OFFERS_WARNING_POSTFIX))
                {
                    int minimumLevel = enchant.get(0);
                    int maximumLevel = enchant.get((enchant.size() < 2) ? 0 : 1);
                    
                    return ItemStackProducerFactory.createItemStackProducer(item,
                            minimum, maximum, damage, minimumLevel, maximumLevel);
                }
                else
                    return ItemStackProducerFactory.createItemStackProducer(item,
        ...
        if (getOfferConfig().contains(itemstackPath))
        {
            if (getOfferConfig().isItemStack(itemstackPath))
                return ItemStackProducerFactory.createItemStackProducer(getOfferConfig()
                        .getItemStack(itemstackPath));
            else
                getLogger().info(
                        "Warning: '" + itemstackPath + "' is not a valid ItemStack! "
                                + OFFERS_WARNING_POSTFIX);
        }
     ...
     *            - the map to populate
     * @param itemNames
     *            - keys representing offers values to be added to the map
     * @param pathHeader
     *            - the parent path of the keys
     */
    private void populateValuesHashMap(HashMap<Item, Tuple> targetMap,
            Set<String> itemNames, String pathHeader)
    {
        for (String name : itemNames)
        {
            ...
            if (itemOrBlockExists(item, name, path, OFFERS_WARNING_POSTFIX))
            {
                ...
                if (rangeExists(range, path, OFFERS_WARNING_POSTFIX))
                    targetMap.put(item,
                            new Tuple(range.get(0),
                                    range.get((range.size() < 2) ? 0 : 1) + 1));
            }
        }
    }
}
