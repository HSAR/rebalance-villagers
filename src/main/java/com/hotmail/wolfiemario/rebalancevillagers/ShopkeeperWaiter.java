...
public class ShopkeeperWaiter implements Runnable
{
    ...
    private net.minecraft.server.v1_7_R3.World mcWorld;
    private RebalanceVillagers plugin;
    
    static int shopkeeperCheckAttempts; // Number of times to check if a
                                        // CUSTOM-spawned
    ...
    public ShopkeeperWaiter(EntityVillager vil, net.minecraft.server.v1_7_R3.World world,
            RebalanceVillagers _plugin)
    {
        villager = vil;
        mcWorld = world;
        plugin = _plugin;
    ...
    public void run()
    {
                        ...
                        .callSyncMethod(plugin, new Callable<Boolean>()
                        {
                            ...
                            public Boolean call() throws Exception
                            {
                                return new Boolean(ShopkeepersHelper
                                        .isShopkeeper(villager.getBukkitEntity()));
                            }
                        }).get().booleanValue();
        ...
        catch (InterruptedException e)
        {
            plugin.getLogger().info(
                    "Thread interruption: No clue how you just managed that."); // Seriously,
                                                                                // assuming
                                                                                // no
                                                                                // reflection,
                                                                                // that
        ...
        {
            plugin.getLogger().info(
                    "Bukkit Scheduler Execution interupted. Dunno if that's a problem.");
            e.printStackTrace();
        }
    }
}
