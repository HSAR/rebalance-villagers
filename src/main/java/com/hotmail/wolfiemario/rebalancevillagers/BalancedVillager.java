...
public class BalancedVillager extends EntityVillager implements IMerchant, NPC
{
    ...
    protected void bo()
    {
        ...
        {
            world.villages.a(MathHelper.floor(locX), MathHelper.floor(locY),
                    MathHelper.floor(locZ));
            profession = 70 + random.nextInt(50);
            village = world.villages.getClosestVillage(MathHelper.floor(locX),
                    MathHelper.floor(locY), MathHelper.floor(locZ), 32);
            if (village == null)
            {
                bX(); // detatchHome
            }
            else
            {
                ...
                a(chunkcoordinates.x, chunkcoordinates.y, chunkcoordinates.z,
                        (int) (village.getSize() * 0.6F));
                if (bz)
                {
                    bz = false;
                    village.b(5);
        ...
        if (!cc() && offerUpdateTicks > 0) // trading related behavior - bS ==
                                           // isTrading, bv == timeUntilReset
        {
            offerUpdateTicks--;
            if (offerUpdateTicks <= 0) // timeUntilReset
            {
                if (needsInitilization) // == needsInitilization - were we
                                        // adding a new offer?
                {
                    ...
                    if (mrList.size() > 1)
                    {
                        ...
                        do
                        {
                            if (!iterator.hasNext())
                                break;
                            MerchantRecipe merchantrecipe = (MerchantRecipe) iterator
                                    .next();
                            if (merchantrecipe.g())
                            {
                                if (offerRemoval)
                                {
                                    if (toRemove == null)
                                        toRemove = new ArrayList<MerchantRecipe>();
                        ...
                        if (toRemove != null)
                        {
                            // if we would remove all of our recipes, reactivate
        ...
        // if we still have no active offer, activate at least one offer so we
        // don't run dry...
    ...
    public boolean a(EntityHuman entityhuman)
    {
        ...
        if (!flag && isAlive() && (!cc() || allowMultivending)
                && (!isBaby() || canTradeChildren)) // alive, adult, and nobody
                                                    // else is trading
        {
            if (!world.isStatic)
            {
    ...
    /**
     * (NMS) EntityVillager method: Offer addition and removal, and riches
     * count, called when a trade is made.
     */
    @Override
    public void a(MerchantRecipe merchantrecipe)
    {
        ...
        if ((merchantrecipe.a((MerchantRecipe) mrList.get(mrList.size() - 1)) || newForAnyTrade)
                && (random.nextInt(100) < newProbability)) // Does this offer
                                                           // equal the last
                                                           // offer on the list?
        {
        ...
        if (merchantrecipe.getBuyItem1().getItem() == currencyItem)
            riches += merchantrecipe.getBuyItem1().count; // increment riches by
                                                          // amount of currency
                                                          // item.
    }
    ...
    /**
     * Attempts to generate the specified number of offers. Limited by the
     * amount of unique offers this villager can actually generate.
     * 
     * @param numOffers
     *            - the number of offers to try generating
     */
    private void generateNewOffers(int numOffers)
    {
        ...
        findAndRemoveAlreadyActiveRecipes(merchantrecipelist); // remove items
                                                               // which are
                                                               // already in the
                                                               // list (fixes
                                                               // villager
    ...
    /**
     * Determines whether or not the specified AbstractOffer is considered for
     * addition to a villager.
     * 
     * @param offer
     *            - the AbstractOffer to check the probability value of
     * @param random
    ...
    /**
     * Creates the MerchantRecipe for a block or item ID, based on the specified
     * map of offer values. Applies the Smart Stacking feature.
     * 
     * @param id
     *            - the item or block ID involved in this offer
     * @param valuesMap
     *            - the map defining the price for the offer
     * @param random
     * @return The MerchantRecipe built based on the parameters.
     */
    private static MerchantRecipe getOffer(Item item, HashMap<Item, Tuple> valuesMap,
            Random random)
    {
        ...
        // Depending on whether we're buying or selling, the input and output
        // are swapped.
        ...
        else
        {
            ...
            else if (value <= 128 || !isCompressible(input)) // if the input
                                                             // can't be
                                                             // compressed, this
                                                             // is the end of
                                                             // the line.
            ...
            else
            {
                ...
                numCompressed = Math.min(numCompressed, 64); // If we cap the
                                                             // blocks at 64, we
                                                             // guarantee
                                                             // amounts up to
                                                             // 640 are
        ...
        if (buyB != null)
            nbttagcompound.set("buyB", buyB.save(new NBTTagCompound()));
    ...
    /**
     * Determines the value of an item or block, correcting for incorrect
     * declarations.
     * 
     * @param id
     *            - the item or block ID involved in this offer
     ...
     *            - the map defining the price for the offer
     * @param random
     * @return 1 if the value could not be found, a positive number if the value
     *         represents the amount in the input slots of a MerchantRecipe, or
     *         a negative number if the value represents the negation of the
     *         amount in the output slot of a MerchantRecipe.
     */
    private static int offerValue(Item item, HashMap<Item, Tuple> valuesMap, Random random)
    {
        Tuple tuple = valuesMap.get(item);
        if (tuple == null)
            return 1;
        if (((Integer) tuple.a()).intValue() >= ((Integer) tuple.b()).intValue())
            ...
            return ((Integer) tuple.a()).intValue()
                    + random.nextInt(((Integer) tuple.b()).intValue()
                            - ((Integer) tuple.a()).intValue());
    }
    
    /**
     * @param id
     *            - the id of the item or block to check
     * @return Whether or not the Smart Compression feature can compress this
     *         item or block.
     */
    ...
    /**
     * Constructs a new BalacedVillager which is identical to the (NMS)
     * EntityVillager vil. Of course, the unique ID will not be the same, and
     * the position still needs to be set.
     * 
     * @param vil
     *            - the (NMS) EntityVillager to clone as a BalancedVillager
     */
    ...
    private void findOutdatedOffers()
    {
        ...
        RebalanceVillagers.debugMsg("Checking for outdated offers in villager at: "
        ...
        while (iIterator.hasNext())
        {
            ...
            {
                if (outdated == null)
                    outdated = new ArrayList<MerchantRecipe>();
        ...
        if (outdated != null)
        {
            ...
            if (mrList.isEmpty())
            {
                offerUpdateTicks = generationTicks; // set offer update ticks to
                                                    // n
                needsInitilization = true;
            }
        }
    ...
    private boolean checkOffer(ItemStack buy1, ItemStack buy2, ItemStack buy3)
    {
        // if he sells things
        if (buy1 != null
                && (buy1.getItem() == currencyItem || getUncompressed(buy1.getItem()) == currencyItem)
                && buy3 != null)
        {
            
            Tuple tuple = sellValues.get(buy3.getItem());
            if (tuple == null)
                return false;
            ...
            // check if we have negative prices (so we sell x items for one
            // currency item)
            boolean neg = min < 0;
            min = Math.abs(min);
            max = Math.abs(max);
            ...
            if (neg)
            {
                ...
                RebalanceVillagers.debugMsg("--> Sell offer [" + min + " < " + amount
                        + " < " + max + "]");
            ...
            else
            {
                ...
                if (buy2 != null)
                    amount = amount
                            + (buy2.count * (isCompressed(buy2.getItem()) ? 9 : 1));
            ...
            // if he buys things
                ...
                && (buy3.getItem() == currencyItem || getUncompressed(buy3.getItem()) == currencyItem)
                && buy1 != null)
        {
            
            Tuple tuple = buyValues.get(buy1.getItem());
            if (tuple == null)
                return false;
            ...
            // check if we have negative prices (so we buy x items for one
            // currency item)
            boolean neg = min < 0;
            min = Math.abs(min);
            max = Math.abs(max);
            ...
            if (neg)
            {
                ...
                RebalanceVillagers.debugMsg("--> Buy offer [" + min + " < " + price
                        + " < " + max + "]");
            ...
            else
            {
                ...
                RebalanceVillagers.debugMsg("+-> Buy offer [" + min + " < " + amount
                        + " < " + max + "]");
                if (amount < min || amount > max)
                    return false;
    ...
    private Item getUncompressed(Item cItem)
    {
        if (compressedForms.containsValue(cItem))
        {
            ...
            {
                if (compressedForms.get(key) == cItem)
                    return key;
            }
        }
        return null;
    ...
    private static HashMap<Item, Item> compressedForms; // private static final
                                                        // Map
    ...
    /*
     * The following setters change properties for all BalancedVillagers. These
     * are invoked when loading the config.
     */
    ...
    /**
     * Populates a MerchantRecipeList with offers from a PotentialOffersList,
     * based on their probability values.
     * 
     * @param merchantrecipelist
     *            - the list to populate
     * @param offers
     *            - the potential offers to populate it with
     * @param random
     *            - I never really understood the reasons for this model of
     *            passing Random...
     */
    @SuppressWarnings("unchecked")
    private static void populateMerchantRecipeList(MerchantRecipeList merchantrecipelist,
            PotentialOffersList offers, Random random)
    {
    ...
    private void findAndRemoveAlreadyActiveRecipes(MerchantRecipeList merchantrecipelist)
    {
        ...
        Iterator<MerchantRecipe> merchantrecipelistIterator = merchantrecipelist
                .iterator();
        while (merchantrecipelistIterator.hasNext())
        {
            ...
            while (iIterator.hasNext())
            {
                ...
                boolean check1 = itemStackEqual(recipeToAdd.getBuyItem1(),
                        activeReceipe.getBuyItem1());
                boolean check2 = itemStackEqual(recipeToAdd.getBuyItem2(),
                        activeReceipe.getBuyItem2());
                boolean check3 = itemStackEqual(recipeToAdd.getBuyItem3(),
                        activeReceipe.getBuyItem3());
                
                if (check1 && check2 && check3)
                {
                    if (doubles == null)
                        doubles = new ArrayList<MerchantRecipe>();
        ...
        if (doubles != null)
            merchantrecipelist.removeAll(doubles);
    }
     ...
     * 
     * @param random
     *            - pass a random object here, so we don't need to create one
     *            every time
    ...
    private void addDefaultRecipes()
    {
        ...
        if (offersByProfession.containsKey(-1))
            populateMerchantRecipeList(merchantrecipelist, offersByProfession.get(-1),
                    random); // Attempt loading user-specified defaults.
        if (merchantrecipelist.isEmpty())
            merchantrecipelist.add(getOffer(Items.GOLD_INGOT, buyValues, random)); // If
                                                                                   // all
                                                                                   // else
                                                                                   // fails...
        for (int l = 0; l < merchantrecipelist.size(); l++)
            mrList.a((MerchantRecipe) merchantrecipelist.get(l));
    }
    ...
    static
    {
        ...
        // Sadly, can't include lapis because it would be considered equal to
        // all dyes.
